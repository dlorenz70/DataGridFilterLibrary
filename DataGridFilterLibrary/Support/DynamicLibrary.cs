...
#pragma warning restore 0219
        ...
        private Expression ParseIdentifier()
        {
            ...
            if (symbols.TryGetValue(token.text, out value) ||
                externals?.TryGetValue(token.text, out value) == true)
            {
    ...
    new[] { left, right });
        ...
        private Exception ParseError(string format, params object[] args) => ParseError(token.pos, format, args);
        private Exception ParseError(int pos, string format, params object[] args) => new ParseException(string.Format(System.Globalization.CultureInfo.CurrentCulture, format, args), pos);
